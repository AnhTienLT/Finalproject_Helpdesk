...
@section('title', 'Sửa Nhật ký Bảo trì')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg border border-gray-200">
        <form action="{{ route('maintenance-logs.update', $maintenanceLog) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="performed_by" id="performed_by" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('performed_by') border-red-300 text-red-900 @enderror">
                        @foreach ($technicians as $tech)
                            <option value="{{ $tech->id }}" {{ old('performed_by', $maintenanceLog->performed_by) == $tech->id ? 'selected' : '' }}>
                                {{ $tech->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="maintenance_date" id="maintenance_date" value="{{ old('maintenance_date', $maintenanceLog->maintenance_date ? \Carbon\Carbon::parse($maintenanceLog->maintenance_date)->format('Y-m-d') : '') }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('maintenance_date') border-red-300 text-red-900 @enderror">
                ...
                <div>
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <input type="number" name="cost" id="cost" value="{{ old('cost', $maintenanceLog->cost) }}" class="block w-full border border-gray-300 rounded-md py-2 pl-3 pr-12 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('cost') border-red-300 text-red-900 placeholder-red-300 @enderror" placeholder="0">
